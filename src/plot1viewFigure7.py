# Plot Figure 7 comparing 1-view schemes using approx. equal sampling
...
def plotFigure7(subfolder, FigFolder, rank, subfigIndx, subFigName, Indx):
...
#    matplotlib.rcParams['text.latex.preamble'] = [r'\usepackage{amsmath}']
    ...
    # Get Frob errors:
    MeanErrEll1Ell2Equal = sp.load(subfolder + '/' + 'TroppMeanFrobErrEll1Ell2Equal.npy')
    MeanErrEll1Ell2EqualEllCutHalf = sp.load(subfolder + '/' + 'TroppMeanFrobErrEll1Ell2EqualEllCutHalf.npy')
    MeanErrEll1Ell2EqualEllCutQuarter = sp.load(subfolder + '/' + 'TroppMeanFrobErrEll1Ell2EqualEllCutQuarter.npy')
    MeanErrEll1Ell2EqualEllCutThreeFourths = sp.load(subfolder + '/' + 'TroppMeanFrobErrEll1Ell2EqualEllCutThreeFourths.npy')
    ...
    Errs = [MeanErrEll1Ell2Equal, MeanErrEll1Ell2EqualEllCutHalf, MeanErrEll1Ell2EqualEllCutQuarter, MeanErrEll1Ell2EqualEllCutThreeFourths,  MeanErrEll1Ell2EqualEllCutMinVar]
    ymin = 1.e99
    ymax = -1.e99
    for Err in Errs:
        emax = max(Err)
    ...
    plt.plot(Tvec - 2*rank, MeanErrEll1Ell2Equal, 'gx-.', markersize=8, markerfacecolor='g', markeredgecolor='g', markeredgewidth=2, linewidth=1.5)
    plt.plot(Tvec - 2*rank, MeanErrEll1Ell2EqualEllCutQuarter, 'rs--', markersize=8, markerfacecolor="None", markeredgecolor='r', markeredgewidth=2, linewidth=1.5)
    ...
    plt.plot(Tvec - 2*rank, MeanErrEll1Ell2EqualEllCutThreeFourths, 'yo:', markersize=8, markerfacecolor='y', markeredgecolor='y', markeredgewidth=2, linewidth=1.5)
    ...
    # Set y axis limits:
    plt.ylim(max(1.e-10, ymin*0.6), min(ymax*1.4, 1.e2))
    plt.xlim(4, 300)
    ...
    if Indx == 7:
        ax.legend(['$l_\mathrm{c} = l_1$', '$l_\mathrm{c} = \lfloor l_1/4 \\rfloor$ ', '$l_\mathrm{c} = \lfloor l_1/2 \\rfloor$', '$l_\mathrm{c} = \lfloor 3 l_1/4 \\rfloor$', '$l_\mathrm{c}$ Min. Var.'], numpoints=1, loc=9, bbox_to_anchor=(0.4, -0.24), ncol=5, frameon=False)
    fig.subplots_adjust(left=0.01, bottom=0.01, right=0.99, top=0.99, wspace=0.4, hspace=0.4)
    fig.savefig(FigFolder + '/Figure7.pdf', format='pdf',bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    for i, matName in enumerate(mats):  
        ...
        # Plot:
        plotFigure7(subfolder, FigFolder, rank, subfigIndices[i], subFigNames[i], i)